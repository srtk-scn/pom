...
function useAsObservableSourceInternal(current, usedByLocalStore) {
    ...
    var _a = __read$1(React__default.useState(function () { return observable(current, {}, { deep: false }); }), 1), res = _a[0];
...
const TableStore = ((props) => {
    ...
    const storeTableDatas = (datas) => {
        const newDatas = datas.map((ele, idx) => {
            let returnData = {};
            for (let key in ele) {
            ...
            returnData = Object.assign(Object.assign({ number: idx + 1, key: `table_data_${idx + 1}` }, returnData), { remove: '삭제' });
            return returnData;
        });
        console.log('newDatas', newDatas);
        setTableDatas(newDatas);
    };
    const storeTableColumns = (columns) => {
        const newColumns = columns.map((el) => {
            const { label, value, key, render } = el, rest = __rest(el, ["label", "value", "key", "render"]);
            return Object.assign({ title: label, dataIndex: value, key: (key ? key : value), render: (render ? render : (text) => text), align: (rest && rest.align ? rest.align : 'center') }, rest);
        });
        if (numbering) {
        ...
        if (options && options.delete) {
            newColumns.push({ title: '삭제', dataIndex: 'remove', key: 'remove', align: 'center',
                render: (text, record) => tableDatas.length >= 1 ? (React__default.createElement(antd.Popconfirm, { title: "\uC815\uB9D0 \uC0AD\uC81C\uD558\uC2DC\uACA0\uC2B5\uB2C8\uAE4C?", placement: "top", okText: "예", cancelText: "아니요", onConfirm: () => deleteTableData(record.key) },
                    React__default.createElement("a", null, text))) : null
            });
        }
    ...
    const deleteTableData = (key) => {
        setTableDatas(tableDatas.filter((item) => item.key !== key));
    };
...
const BasicTable = observer((props) => {
    const { url, columns, datas, numbering = true } = props, rest = __rest(props, ["url", "columns", "datas", "numbering"]);
    const tableStore = TableStore(props);
    console.log(tableStore);
    const tableColumns = columns.map(el => {
        return Object.assign({ title: el.label, dataIndex: el.value, key: el.value, align: 'center' }, el);
    });
    if (numbering) {
    ...
    tableColumns.push({ title: '삭제', dataIndex: 'remove', key: 'remove',
        render: (text, record) => dataSources.length >= 1 ? (React__default.createElement(antd.Popconfirm, { title: "\uC815\uB9D0 \uC0AD\uC81C\uD558\uC2DC\uACA0\uC2B5\uB2C8\uAE4C?", placement: "top", okText: "예", cancelText: "아니요", onConfirm: () => store.deleteData(record.key) },
            React__default.createElement("a", null, text))) : null
    });
    const dataSources = datas.map((ele, idx) => {
        let returnData = {};
        for (let key in ele) {
            if (ele.hasOwnProperty(key)) {
                returnData = Object.assign(Object.assign({}, returnData), { [key]: ele[key] });
    ...
    return (React__default.createElement(React__default.Fragment, null,
        React__default.createElement(antd.Table, Object.assign({ columns: store.tableColumns, dataSource: store.dataSources, rowKey: record => record.uid }, rest)),
        React__default.createElement(antd.Table, Object.assign({ columns: tableStore.getTableColumns(), dataSource: tableStore.getTableDatas(), rowKey: record => record.uid }, rest))));
});

const App = (props) => {
    const { columns, datas } = props, rest = __rest(props, ["columns", "datas"]);
    return (React__default.createElement(BasicTable, Object.assign({ url: 'https://test.url', columns: columns, datas: datas }, rest)));
};
